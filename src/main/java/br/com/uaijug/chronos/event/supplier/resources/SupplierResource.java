...
public class SupplierResource {
	...
	@Path("/{id:[0-9][0-9]*}")
	@Produces(MediaType.APPLICATION_JSON)
	public Supplier lookupSupplierById(@PathParam("id") long id) {
	...
	public Response createSupplier(Supplier supplier) {
		...
		try {
			// Validates supplier using bean validation
	...
	/**
	 * <p>
	 * Validates the given Supplier variable and throws validation exceptions
	 * based on the type of error. If the error is standard bean validation
	 * errors then it will throw a ConstraintValidationException with the set of
	 * the constraints violated.
	 * </p>
	 * <p>
	 * If the error is caused because an existing supplier with the same email is
	 * registered it throws a regular validation exception so that it can be
	 * interpreted separately.
	 * </p>
	 * 
	 * @param supplier
	 *            Supplier to be validated
	 * @throws ConstraintViolationException
	 *             If Bean Validation errors exist
	 * @throws ValidationException
	 *             If supplier with the same email already exists
	 */
	private void validateSupplier(Supplier supplier)
			throws ConstraintViolationException, ValidationException {
		// Create a bean validator and check for issues.
		Set<ConstraintViolation<Supplier>> violations = validator
				.validate(supplier);
