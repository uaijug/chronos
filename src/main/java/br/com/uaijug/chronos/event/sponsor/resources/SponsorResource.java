...
public class SponsorResource {
	...
	@Path("/{id:[0-9][0-9]*}")
	@Produces(MediaType.APPLICATION_JSON)
	public Sponsor lookupSponsorById(@PathParam("id") long id) {
	...
	public Response createSponsor(Sponsor sponsor) {
		...
		try {
			// Validates sponsor using bean validation
			validateSponsor(sponsor);

			registration.register(sponsor);
	...
	/**
	 * <p>
	 * Validates the given Sponsor variable and throws validation exceptions
	 * based on the type of error. If the error is standard bean validation
	 * errors then it will throw a ConstraintValidationException with the set of
	 * the constraints violated.
	 * </p>
	 * <p>
	 * If the error is caused because an existing sponsor with the same email is
	 * registered it throws a regular validation exception so that it can be
	 * interpreted separately.
	 * </p>
	 * 
	 * @param sponsor
	 *            Sponsor to be validated
	 * @throws ConstraintViolationException
	 *             If Bean Validation errors exist
	 * @throws ValidationException
	 *             If sponsor with the same email already exists
	 */
	private void validateSponsor(Sponsor sponsor)
			throws ConstraintViolationException, ValidationException {
		// Create a bean validator and check for issues.
		Set<ConstraintViolation<Sponsor>> violations = validator
				.validate(sponsor);
