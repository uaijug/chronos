...
public class EventSessionRegistrationImpl extends PersistenceBase<EventSession, Long> implements EventSessionRegistration {
	...
	public Boolean register(EventSession eventSession) {
		...
		if (exists(eventSession.getId())) {
			...
			eventSession.setCreateTime(new Date());
			eventSession.setUpdateTime(new Date());

			if (eventSession.getId() != null) {
				log.info("Updating " + eventSession.getId());
				update(eventSession);
			} else {
				log.info("Registering " + eventSession.getId());
				save(eventSession);
			}
		}
		eventEventSessionSrc.fire(eventSession);
		return saved;
	}
}
