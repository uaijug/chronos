...
public class MaterialResource {
	...
	@Path("/{id:[0-9][0-9]*}")
	@Produces(MediaType.APPLICATION_JSON)
	public Material lookupMaterialById(@PathParam("id") long id) {
...
/*	@GET
	@Path("/s/{slug:[a-z][a-z]*}")
	@Produces(MediaType.APPLICATION_JSON)
	public Material lookupMaterialBySlug(@PathParam("slug") String slug) {
		Material material = repository.findBySlug(slug);
		if (material == null) {
			throw new WebApplicationException(Response.Status.NOT_FOUND);
		}
		return material;
	...
	@Produces(MediaType.APPLICATION_JSON)
	public Response createMaterial(Material material) {

		Response.ResponseBuilder builder = null;

		try {
			// Validates material using bean validation
	...
	/**
	 * <p>
	 * Validates the given Material variable and throws validation exceptions
	 * based on the type of error. If the error is standard bean validation
	 * errors then it will throw a ConstraintValidationException with the set of
	 * the constraints violated.
	 * </p>
	 * <p>
	 * If the error is caused because an existing material with the same email is
	 * registered it throws a regular validation exception so that it can be
	 * interpreted separately.
	 * </p>
	 * 
	 * @param material
	 *            Material to be validated
	 * @throws ConstraintViolationException
	 *             If Bean Validation errors exist
	 * @throws ValidationException
	 *             If material with the same email already exists
	 */
