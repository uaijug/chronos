...
public class EventMainRegistrationImpl extends PersistenceBase<EventMain, Long>
	...
	public Boolean register(EventMain eventMain) {
		...
		if (exists(eventMain.getId())) {
			...
			eventMain.setCreateTime(new Date());
			eventMain.setUpdateTime(new Date());

			if (eventMain.getId() != null) {
				log.info("Updating " + eventMain.getDescription());
				update(eventMain);
			} else {
				log.info("Registering " + eventMain.getDescription());
				save(eventMain);
			}
		}
		eventEventMainSrc.fire(eventMain);
		return saved;
