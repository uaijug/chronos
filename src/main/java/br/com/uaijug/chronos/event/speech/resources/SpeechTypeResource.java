...
@Path("/speechs")
@RequestScoped
public class SpeechTypeResource {
	...
	/** The repository. */
	@Inject
	private SpeechTypeRepository repository;

	/** The registration. */
	@Inject
	SpeechTypeRegistration registration;

	/**
	 * List all speech types.
	 *
	 * @return the list
	 */
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	public List<SpeechType> listAllSpeechTypes() {
	...
	@Path("/{id:[0-9][0-9]*}")
	@Produces(MediaType.APPLICATION_JSON)
	public SpeechType lookupSpeechById(@PathParam("id") long id) {
		SpeechType speechType = repository.findById(id);
		if (speechType == null) {
	...
	@Produces(MediaType.APPLICATION_JSON)
	public Response createSpeech(SpeechType speechType) {
		...
		try {
			...
			registration.register(speechType);
	 ...
	 * @throws ValidationException             If speech with the same email already exists
	 */
	private void validateSpeechType(SpeechType speechType)
			throws ConstraintViolationException, ValidationException {
		// Create a bean validator and check for issues.
		Set<ConstraintViolation<SpeechType>> violations = validator
				.validate(speechType);
