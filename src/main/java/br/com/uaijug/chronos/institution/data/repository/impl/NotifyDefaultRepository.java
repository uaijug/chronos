...
public class NotifyDefaultRepository extends PersistenceBase<Notify, Long> implements NotifyRepository {
    ...
    public List<Notify> findAllOrderedByName() {
        ...
        Root<Notify> notify = criteria.from(Notify.class);
        // Swap criteria statements if you would like to try out type-safe criteria queries, a new
        // feature in JPA 2.0
        // criteria.select(notify).orderBy(cb.asc(notify.get(Notify_.name)));
        criteria.select(notify).orderBy(cb.asc(notify.get("subject")));
