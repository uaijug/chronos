...
public class CashFlowDefaultRepository extends PersistenceBase<CashFlow, Long> implements CashFlowRepository {
    ...
    public List<CashFlow> findAllOrderedByName() {
        ...
        Root<CashFlow> cashFlow = criteria.from(CashFlow.class);
        // Swap criteria statements if you would like to try out type-safe criteria queries, a new
        // feature in JPA 2.0
        // criteria.select(cashFlow).orderBy(cb.asc(cashFlow.get(CashFlow_.name)));
        criteria.select(cashFlow).orderBy(cb.asc(cashFlow.get("description")));
