 ...
 */
package br.com.uaijug.chronos.studygroup.service.impl;
	...
	public Boolean register(StudyGroup studyGroup) {
		...
		if (exists(studyGroup.getId())) {
			...
			if (studyGroup.getId() != null) {
				log.info("Updating " + studyGroup.getDescription());
				update(studyGroup);
			} else {
				log.info("Registering " + studyGroup.getDescription());
				save(studyGroup);
			}
		}
		studyGroupEventSrc.fire(studyGroup);
		return saved;
	}
}
