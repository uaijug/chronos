...
public class TaskCategoryResource {
	...
	@Path("/{id:[0-9][0-9]*}")
	@Produces(MediaType.APPLICATION_JSON)
	public TaskCategory lookupTaskCategoryById(@PathParam("id") long id) {
		TaskCategory taskCategory = repository.findById(id);
	...
	@Produces(MediaType.APPLICATION_JSON)
	public Response createTaskCategory(TaskCategory taskCategory) {

		Response.ResponseBuilder builder = null;

		try {
			// Validates taskCategory using bean validation
			validateTaskCategory(taskCategory);

			registration.register(taskCategory);
	...
	/**
	 * <p>
	 * Validates the given TaskCategory variable and throws validation exceptions
	 * based on the type of error. If the error is standard bean validation
	 * errors then it will throw a ConstraintValidationException with the set of
	 * the constraints violated.
	 * </p>
	 * <p>
	 * If the error is caused because an existing taskCategory with the same email is
	 * registered it throws a regular validation exception so that it can be
	 * interpreted separately.
	 * </p>
	 * 
	 * @param taskCategory
	 *            TaskCategory to be validated
	 * @throws ConstraintViolationException
	 *             If Bean Validation errors exist
	 * @throws ValidationException
	 *             If taskCategory with the same email already exists
	 */
	private void validateTaskCategory(TaskCategory taskCategory)
			throws ConstraintViolationException, ValidationException {
		// Create a bean validator and check for issues.
		Set<ConstraintViolation<TaskCategory>> violations = validator
