...
public class TaskGroupCategoryResource {
	...
	@Path("/{id:[0-9][0-9]*}")
	@Produces(MediaType.APPLICATION_JSON)
	public TaskGroupCategory lookupTaskGroupCategoryById(@PathParam("id") long id) {
		TaskGroupCategory taskGroupCategory = repository.findById(id);
	...
	@Produces(MediaType.APPLICATION_JSON)
	public Response createTaskGroupCategory(TaskGroupCategory taskGroupCategory) {

		Response.ResponseBuilder builder = null;

		try {
			// Validates taskGroupCategory using bean validation
			validateTaskGroupCategory(taskGroupCategory);

			registration.register(taskGroupCategory);
	...
	/**
	 * <p>
	 * Validates the given TaskGroupCategory variable and throws validation exceptions
	 * based on the type of error. If the error is standard bean validation
	 * errors then it will throw a ConstraintValidationException with the set of
	 * the constraints violated.
	 * </p>
	 * <p>
	 * If the error is caused because an existing taskGroupCategory with the same email is
	 * registered it throws a regular validation exception so that it can be
	 * interpreted separately.
	 * </p>
	 * 
	 * @param taskGroupCategory
	 *            TaskGroupCategory to be validated
	 * @throws ConstraintViolationException
	 *             If Bean Validation errors exist
	 * @throws ValidationException
	 *             If taskGroupCategory with the same email already exists
	 */
	private void validateTaskGroupCategory(TaskGroupCategory taskGroupCategory)
			throws ConstraintViolationException, ValidationException {
		// Create a bean validator and check for issues.
		Set<ConstraintViolation<TaskGroupCategory>> violations = validator
				.validate(taskGroupCategory);
