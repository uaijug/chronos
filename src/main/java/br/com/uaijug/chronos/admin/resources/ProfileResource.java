...
public class ProfileResource {
	...
	@Path("/{id:[0-9][0-9]*}")
	@Produces(MediaType.APPLICATION_JSON)
	public Profile lookupProfileById(@PathParam("id") long id) {
	...
	public Response createProfile(Profile profile) {
		...
		try {
			// Validates profile using bean validation
	...
	/**
	 * <p>
	 * Validates the given Profile variable and throws validation exceptions
	 * based on the type of error. If the error is standard bean validation
	 * errors then it will throw a ConstraintValidationException with the set of
	 * the constraints violated.
	 * </p>
	 * <p>
	 * If the error is caused because an existing profile with the same email is
	 ...
	 *             If profile with the same email already exists
	 */
	private void validateProfile(Profile profile)
			throws ConstraintViolationException, ValidationException {
		// Create a bean validator and check for issues.
		Set<ConstraintViolation<Profile>> violations = validator
				.validate(profile);
