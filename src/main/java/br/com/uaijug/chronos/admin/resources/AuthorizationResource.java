...
public class AuthorizationResource {
	...
	@Path("/{id:[0-9][0-9]*}")
	@Produces(MediaType.APPLICATION_JSON)
	public Authorization lookupAuthorizationById(@PathParam("id") long id) {
	...
	@Produces(MediaType.APPLICATION_JSON)
	public Response createAuthorization(Authorization authorization) {

		Response.ResponseBuilder builder = null;

		try {
			// Validates authorization using bean validation
			validateAuthorization(authorization);

			registration.register(authorization);
	...
	/**
	 * <p>
	 * Validates the given Authorization variable and throws validation exceptions
	 * based on the type of error. If the error is standard bean validation
	 * errors then it will throw a ConstraintValidationException with the set of
	 * the constraints violated.
	 * </p>
	 * <p>
	 * If the error is caused because an existing authorization with the same email is
	 * registered it throws a regular validation exception so that it can be
	 * interpreted separately.
	 * </p>
	 * 
	 * @param authorization
	 *            Authorization to be validated
	 * @throws ConstraintViolationException
	 *             If Bean Validation errors exist
	 * @throws ValidationException
	 *             If authorization with the same email already exists
	 */
